...
# ========== TOP 3 + FUTURE ==========
    ...
    # --- SEMPRE preparar DataFrame ---
    # Renomeia colunas para o esperado pelo agent
    df_combined_ai = df_combined.copy()
    # Se o seu CSV tem 'input', renomeie para 'model_input'
    if 'input' in df_combined_ai.columns:
        df_combined_ai = df_combined_ai.rename(columns={'input': 'model_input'})
    # Se faltar 'date', crie a partir de 'month_year'
    if 'date' not in df_combined_ai.columns and 'month_year' in df_combined_ai.columns:
        df_combined_ai['date'] = df_combined_ai['month_year']
    # Garante tipo string para datas
    for col in ['date', 'customer']:
        if col in df_combined_ai.columns:
    ...
    for col in df_combined_ai.columns:
        if col not in ['forecast_ai', 'customer_demand', 'forecast_deviation_abs']:
            df_combined_ai[col] = df_combined_ai[col].astype(str)
    missing_cols = [col for col in columns_for_ai if col not in df_combined_ai.columns]
    ...
    def is_seg_question(user_query):
        ...
        return any(kw in user_query.lower() for kw in allowed)
    ...
    def get_guarded_answer(user_query):
        ...
        for greet in greetings:
            if greet in q:
                return None
        if any(qs in q for qs in [
            'who is seg', 'what is seg', 'what does seg mean', 'about seg'
        ]):
            return (
        ...
        if any(qs in q for qs in [
            'who are you', 'what are you', 'what is you', 'who is this', 'who is the assistant',
            'who am i talking to', 'what can you do', 'quem é você', 'o que é você', 'o que você faz', 'o que vc pode me ajudar'
        ]):
            return (
                "I am the official AI assistant of SEG Automotive, developed to support users with business intelligence, "
                "model comparison, and data analysis within the SEG Automotive context.\n\n"
    ...
    user_query = st.text_input(
        "Ask a question about your data (e.g., 'What is the best forecast for customer1.0 in May 2025?')",
        key="ai_input"
    )
        ...
        if st.button("🔍 Ask AI") and user_query.strip():
            ...
            try:
                if is_data_question:
                    ...
                    custom_prompt = """
You are the official AI assistant for SEG Automotive, designed to support users in business intelligence and predictive analytics for the automotive sector.

Always respond in English. The main date column is `date`, always formatted as 'YYYY-MM' (e.g., '2025-05'). Never expect a day value, never use 'YYYY-MM-DD'.

- **customer**: client being analyzed.
- **model_input**: variables used (e.g., cd, po, ipca, selic)
- **date**: always 'YYYY-MM'
- **customer_demand**: actual demand (units)
- **forecast_ai**: predicted demand (units)
- **forecast_deviation_abs**: absolute error (percentage)
- **data_type**: 'test' (historical) or 'future' (forecasted)

Business logic:
- Use only the `date` column with 'YYYY-MM' for monthly filtering, never expect days.
- The best forecast/model/customer = smallest average forecast_deviation_abs.
- Never use monetary value, only units.

Examples:
- "What is the best forecast for customer1.0 in May 2025?" → filter by `customer == 'customer1.0'` and `date == '2025-05'`.
- "Compare models for customer2.0 in 2024." → filter for year '2024' on `date`.

If no data exists for the filtered selection, reply: "There is no forecast data available for [criteria] in the provided dataset."

Always finish answers with:
Final Answer: (and the answer, table or chart if needed)
                    """
